...
const CancelRequestModal = ({ isOpen, onClose, item, onConfirm, loading }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden border-2 border-orange-100">
        <div className="p-6 text-center">
            ...
            <p className="text-sm text-gray-600 mb-6 leading-relaxed">
                You are about to report <strong>{item.provider}</strong> as cancelled. This will notify an Admin to approve the removal.
            </p>
            ...
            <div className="flex gap-3">
                ...
                <button 
                    ...
                    disabled={loading}
                    className="flex-[1.5] py-2 text-xs font-bold text-white bg-orange-600 rounded-lg hover:bg-orange-700 shadow-md flex items-center justify-center gap-2"
                >
                    {loading ? 'Processing...' : 'Yes, Request Cancel'} <ArrowRight size={14} />
                </button>
            </div>
        </div>
      </div>
