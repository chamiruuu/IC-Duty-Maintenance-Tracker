...
const CompletionModal = ({
  ...
  const minutesDifference = scheduledEnd && completionTime 
    ? scheduledEnd.diff(completionTime, 'minute') 
    : 0;

  // UPDATED: Only flag as 'Early' if finished MORE than 5 minutes before schedule.
  // 5 minutes or less is considered "On Time/Normal".
  ...
  const getFinishContent = () => {
    // 1. Part of the Game Logic (Highest Priority)
    if (isPartGame) {
      ...
      >
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span
              className={`block text-sm font-bold ${isChecked ? textColor : "text-gray-900"}`}
            >
              {title}
            </span>
            {copyText && (
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="space-y-3">
            ...
            {isEarly ? (
              <>
                {/* Hide Manual Open if Part of the Game */}
                {!isPartGame &&
                  renderCheckItem(
                    "manualOpen",
              ...
              <>
                {/* Standard Manual Open: Only if NOT Part of Game and IS Extended */}
                {!isPartGame &&
                  isExtended &&
                  renderCheckItem(
                    "manualOpen",
