...
const ProviderManagerModal = ({ isOpen, onClose, canManageAdmin }) => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden border border-gray-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">
                Configure provider list & status
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex gap-3 bg-gray-50/80 shrink-0 items-center">
          <div className="relative flex-1 group">
            <Search
              className="absolute left-3.5 top-2.5 text-gray-400 group-focus-within:text-indigo-500 transition-colors"
              size={16}
            />
            <input
              type="text"
          ...
          {/* ONLY SHOW IF ADMIN/LEADER */}
          {!isAdding && canManageAdmin && (
            <button
              onClick={() => setIsAdding(true)}
              className="px-4 py-2 bg-gray-900 hover:bg-black text-white text-sm font-bold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-gray-200 hover:shadow-xl active:scale-95"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50/50">
            ...
            <div className="mb-4 p-4 bg-white border border-indigo-100 rounded-2xl shadow-sm animate-in slide-in-from-top-4 fade-in duration-300">
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-end">
                ...
                <div className="flex items-center justify-between w-full sm:w-auto gap-3">
                  <label
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border cursor-pointer transition-all select-none ${newProvider.is_in_house ? "bg-indigo-50 border-indigo-200 text-indigo-700" : "bg-white border-gray-200 text-gray-500 hover:border-gray-300"}`}
                  >
                    <input
                      type="checkbox"
                      checked={newProvider.is_in_house}
                      onChange={(e) =>
                        setNewProvider({
                          ...newProvider,
                          is_in_house: e.target.checked,
                        })
                      }
                      className="hidden"
                    />
                  ...
                  <button
                    onClick={handleAdd}
                    className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg transition-colors shadow-md"
                  >
                    Save
                  </button>
                </div>
          ...
          {loading ? (
            ...
            filteredProviders.map((p) => (
              <div
                key={p.id}
                className={`group relative bg-white p-3 rounded-xl border transition-all duration-200 ${editingId === p.id ? "border-indigo-500 ring-1 ring-indigo-500 shadow-md z-10" : "border-gray-200 hover:border-indigo-200 hover:shadow-sm"}`}
              >
                {editingId === p.id ? (
                  // --- EDIT MODE ---
                  <div className="flex flex-col sm:flex-row items-center gap-3 animate-in fade-in">
                    <div className="flex-1 w-full">
                      <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block sm:hidden">
                        Name
                      </label>
                      <input
                      ...
                      <div className="flex gap-1">
                        <button
                          onClick={() => {
                            ...
                            const is_in_house = document.getElementById(
                              `edit-house-${p.id}`,
                            ).checked;
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      {/* Avatar */}
                      <div
                        className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black shadow-sm transition-colors ${p.is_in_house ? "bg-indigo-100 text-indigo-600" : "bg-gray-100 text-gray-500"}`}
                      >
                    ...
                    {/* Action Buttons (Visible on Hover) ONLY IF ADMIN */}
                    {canManageAdmin && (
        ...
        <div className="bg-gray-50 p-3 border-t border-gray-100 text-center">
          <p className="text-[10px] text-gray-400 font-medium">
            {filteredProviders.length} providers loaded • Changes reflect
            immediately in Entry Modal
          </p>
        </div>
      </div>
    </div>
