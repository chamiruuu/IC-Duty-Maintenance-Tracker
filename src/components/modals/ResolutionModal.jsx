...
import { X, CheckCircle2, Clock, Copy, Loader2, AlertTriangle, FileText, Send, Check } from 'lucide-react';
import { DatePicker } from '@mui/x-date-pickers/DatePicker';
import { TimePicker } from '@mui/x-date-pickers/TimePicker';
import dayjs from 'dayjs';
import CopyButton from '../CopyButton';

const ResolutionModal = ({ isOpen, onClose, item, onExtend, onComplete, loading }) => {
  const [mode, setMode] = useState('select'); // 'select', 'extend'
  const [extensionType, setExtensionType] = useState('time'); // 'time', 'notice'
  ...
  useEffect(() => {
    if (isOpen && item) {
      ...
      setChecklist({ boUpdate: false, notifyMerchant: false, redmineUpdate: false });
    }
  }, [isOpen, item]);
  ...
  const isSopComplete = checklist.boUpdate && checklist.notifyMerchant && checklist.redmineUpdate;

  // --- GENERATE SCRIPTS ---
  const getInternalGroupMsg = () => {
    const timeInfo = extensionType === 'notice' 
      ? "until further notice" 
      : `to ${newEndTime?.format('HH:mm')}`;
      
    return `Maintenance time update for [${item.provider}], BO8.2 announcement has been updated (${timeInfo}).`;
  };

  const getAnnouncementTitle = () => {
    if (extensionType === 'notice') {
        return `【${item.provider}】 maintenance extended until further notice`;
    }
    return `【${item.provider}】 maintenance extended to ${newEndTime?.format('HH:mm')} (GMT+8)`;
  };

  const getAnnouncementBody = () => {
    const timeStr = extensionType === 'notice' 
        ? "until further notice" 
        : `${newEndTime?.format('YYYY-MM-DD HH:mm')} (GMT+8)`;
  ...
  // Helper to render a Checkable Step Card
  const renderStep = (key, number, text) => {
    const isChecked = checklist[key];
    return (
        <div 
            onClick={() => toggleCheck(key)}
            className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-all duration-200 group ${
                isChecked 
        ...
        >
            ...
            <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-colors ${
                ...
                {isChecked ? <Check size={14} strokeWidth={3} /> : <span className="text-xs font-bold">{number}</span>}
            </div>
            
            {/* Text */}
            <span className={`text-xs leading-5 select-none ${isChecked ? 'font-medium' : ''}`}>
                {text}
            </span>
        </div>
    );
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
          <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wide flex items-center gap-2">
             <Clock size={16} className="text-red-500" /> Maintenance Overdue Resolution
          </h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                        ...
                        <div className="text-center">
                            <span className="block font-bold text-emerald-900 text-lg">Completed</span>
                            <span className="text-xs text-emerald-600">Game is open & transfers working</span>
                        </div>
                    </button>
                    ...
                    >
                        ...
                        <div className="text-center">
                            <span className="block font-bold text-blue-900 text-lg">Extend Time</span>
                            <span className="text-xs text-blue-600">Provider needs more time</span>
                        </div>
                    </button>
                </div>
            )}

            {/* --- SCREEN 2: EXTENSION DETAILS --- */}
            {mode === 'extend' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="space-y-2">
                                ...
                                {renderStep(
                                    'boUpdate', 
                                    '1', 
                                    <>Update <strong>BO8.2</strong> Announcement.<br/>{extensionType === 'notice' ? 'Check "Until Further Notice".' : 'Update the "End Time".'}</>
                                )}

                                {renderStep(
                                    'notifyMerchant', 
                                    '2', 
                                    <>Notify Merchant via Robot's BO Select:<br/><strong>【IC-Maintenance&Promo of providers】</strong></>
                                )}

                                {renderStep(
                                    'redmineUpdate', 
                                    '3', 
                                    <>Update <strong>Redmine (Note 4)</strong> and Maintenance Schedule.</>
                                )}
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-gray-400">ANNOUNCEMENT TITLE</label>
                                <div className="flex gap-2">
                                    <div className="flex-1 bg-gray-50 border border-gray-200 rounded p-2 text-xs font-mono text-gray-700 truncate">{getAnnouncementTitle()}</div>
                    ...
                    <div className="flex gap-3 pt-2 border-t border-gray-100">
                        <button onClick={() => setMode('select')} className="px-4 py-2 text-xs font-bold text-gray-500 hover:bg-gray-100 rounded-lg">Back</button>
                        <button 
                            onClick={handleConfirmExtension} 
                            disabled={loading || !isSopComplete}
                            className={`flex-1 py-2 text-xs font-bold rounded-lg shadow-sm flex items-center justify-center gap-2 transition-all ${
                                isSopComplete 
